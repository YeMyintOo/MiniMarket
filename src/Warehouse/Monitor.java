...
public class Monitor extends JPanel {
	...
	public Monitor() {
		...
		showB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				db = new DriverClass();
				String sql = "select * from item i,warehouse w where i.id=w.id";
				...
				try {
					...
					while (rs.next()) {
						total = total + (rs.getInt("countNumber") * rs.getDouble("buyPrice"));
						PItem item = new PItem(rs.getString("id"), rs.getString("name"), rs.getString("arrivalDate"),
								rs.getInt("countNumber"), rs.getInt("buyPrice"));
						plistP.add(item);
					}
