...
public class Search extends JPanel {
	...
	final String[] year = new String[] {"2015", "2016", "2017", "2018", "2019", "2030", "2031", "2032", "2033",
			"2034", "2035", "2036" };
	...
	private String csql = "select * from warehouse w where w.id>=1 ";
	private String sql = "select * from warehouse w where w.id>=1 ";
	...
	public Search() {
		...
		sPriceL.setEnabled(false);
		sPriceF = new JTextField();
		sPriceF.setEnabled(false);
		...
		searchB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				csql = "select * from warehouse w,item i where w.id>=1 and i.id=w.id";
				...
				if (bPriceL.isSelected()) {
					String key = " and w.buyPrice='" + Double.parseDouble(bPriceF.getText().trim()) + "'";
					csql = csql + key;
				} else {
					csql = csql;
				}
				...
				if (countL.isSelected()) {
					String key = " and w.countNumber='" + Integer.parseInt(countF.getText().trim()) + "'";
					csql = csql + key;
				} else {
					csql = csql;
				}
		...
		aDateL.addItemListener(new ItemListener(){

			@Override
			public void itemStateChanged(ItemEvent arg0) {
				if(aDateL.isSelected()){
					eDateL.setEnabled(false);
				}else{
					eDateL.setEnabled(true);
				}
			}
		});
		eDateL.addItemListener(new ItemListener(){

			@Override
			public void itemStateChanged(ItemEvent arg0) {
				if(eDateL.isSelected()){
					aDateL.setEnabled(false);
				}else{
					aDateL.setEnabled(true);
				}
			}
