...
public class Edit extends JPanel implements ActionListener {
	...
	public Edit() {
		...
		empF = new JTextField();
		empB = new CButton("...");
		p.add(empF, BorderLayout.CENTER);
		p.add(empB, BorderLayout.EAST);
		...
		empB.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				showList = new ShowEmployeeList();
				showList.closeB.addActionListener(new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent e) {
						empF.setText(showList.code);
					}
				});
			}
		});
		empF.addKeyListener(new MyKeyListener() {
	...
	public void loadData() {
		db=new DriverClass();
		try {
			String sql = "SELECT * FROM employee e,employeerank r where e.eid=r.rid and e.eid='" + empF.getText().trim() + "'";
			try {
				Connection c = db.getConnection();
				Statement stmt = c.createStatement();
				ResultSet rs = stmt.executeQuery(sql);
				while (rs.next()) {
					nrcF.setText(rs.getString("nrc"));
					nameF.setText(rs.getString("name"));
					rankF.setSelectedIndex(rs.getInt("erid")-1);
					if(rs.getBoolean("gender")){
						maleR.setSelected(true);
					}else{
						femaleR.setSelected(true);
					}
					phF.setText(""+rs.getInt("phone"));
